...
import { ArrowRight, CheckCircle2, Map, Quote, Star, TrendingUp, Users, BadgeCheck, Sparkles, Navigation } from 'lucide-react';
...
export default function HomeDeferredSections({
  ...
  const popularTrips = [...featuredTrips].sort((a, b) => (b.ratingsCount || 0) - (a.ratingsCount || 0)).slice(0, 3);
  const trendingDestinations = [...featuredTrips].slice(0, 4);
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-lg shadow-slate-900/5 sm:p-8">
            ...
            <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-3xl bg-slate-50 p-5">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {[
                    { name: 'Ava', location: 'Singapore', text: 'Best trip planning experience I have used.' },
                    { name: 'Noah', location: 'London', text: 'Fast, polished, and simple to browse on mobile.' },
                    { name: 'Mia', location: 'Sydney', text: 'The compare flow and details are genuinely useful.' },
                  ].map((item) => (
              ...
              <div className="rounded-3xl bg-slate-50 p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {trendingDestinations.map((trip) => (
                    <Link key={trip.destination} to={`/explore?q=${encodeURIComponent(trip.destination.split(',')[0])}`} className="group overflow-hidden rounded-2xl bg-white shadow-sm">
            ...
            <div className="mt-6 rounded-3xl border border-slate-100 bg-white p-5">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {[
                  'The quickest way to compare trips before booking.',
                  'A polished experience that feels premium on mobile.',
                  'Helpful filters and curated suggestions save time.',
                ].map((text, index) => (
                  <div key={index} className="rounded-2xl bg-slate-50 p-4">
                    <p className="font-bold text-slate-900">Highlight {index + 1}</p>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {featuredVibes.map((vibe) => (
              <button
                key={vibe.title}
                type="button"
                onClick={() => navigate(`/explore?q=${encodeURIComponent(vibe.destination)}`)}
      ...
      <section className="relative py-24 overflow-hidden bg-primary-900">
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-br from-primary-800 to-primary-900 opacity-100" />
